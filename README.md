...
**OCR データと XML 抽出データをマージし、病院口コミレビューを構造化 Excel として出力するスクリプト。**

---

## 概要

Hokuto アプリ等から取得した病院口コミデータは、以下の 2 種類のソースから得られます：

| ソース | 強み | 弱み |
|--------|------|------|
| **OCR データ** (JSONL) | 「良い点」「気になる点」の**見出し構造**を保持 | テキスト精度が低い（OCR 誤字） |
| **XML 抽出データ** (TXT) | **本文テキストが正確** | 見出し（良い点/気になる点）が欠落 |

本スクリプトは **両方のデータを類似度マッチングで突き合わせ**、OCR の見出し構造 + XML の高精度テキストを組み合わせた **ベスト・オブ・ボス** のレビューデータを生成します。
...
| 機能 | 説明 |
|------|------|
| **OCR × XML マージ** | 年度・学年・参加形態でメタデータをマッチし、`SequenceMatcher` でテキスト類似度 30% 以上のペアを結合 |
| **良い点 / 気になる点の自動分割** | OCR の見出し比率を使って XML テキストを「良い点」「気になる点」に比率分割 |
| **分裂レビューの自動結合** | ページまたぎで分裂した同一レビューを、テキスト末尾↔先頭の重なり検出で自動結合 |
| **重複レビュー除去** | ほぼ同一のレビュー（類似度 92% 以上）は長い方を残して自動除去 |
| **ノイズ除去** | 「口コミの問題を報告」以降のゴミ、Markdown 見出し、画像マーカー、`.png` 行等を自動フィルタ |
| **中国語前置き除去対応** | OCR が出力しがちな「以下是图片中的文字内容」等を自動除去 |
| **大学名正規化** | 「大学」「医科大学」「医学部」を含む候補を抽出し、ノイズ混入を防止 |
| **病院情報抽出** | XML からマッチ者数/定員、強い科、病床数、給与等を自動抽出 |
| **Excel 出力** | ヘッダー装飾・列幅調整・テキスト折り返し・オートフィルター付きの `.xlsx` を生成 |
...
```
hokuto_scrape.py                  hokuto_scrape_xml_dump.py
  │ (スクリーンショット)               │ (XML/テキスト抽出)
  ▼                                  ▼
OCR (HunyuanOCR / PaddleOCR)     XML 抽出テキスト (.txt)
  │                                  │
  ▼                                  │
OCR 結果 (.jsonl)                    │
  │                                  │
  └──────────┬───────────────────────┘
             ▼
   hokuto_marge_ocr_xml_reviews.py
             │
             ▼
     構造化 Excel (.xlsx)
     ├── Reviews シート（良い点/気になる点/メタデータ）
     └── Hospital Info シート（病院基本情報）
```
...
```bash
python hokuto_marge_ocr_xml_reviews.py <OCR_JSONL> <XML_TXT> [OUTPUT_XLSX]
```
...
| 引数 | 必須 | 説明 |
|------|------|------|
| `OCR_JSONL` | ✅ | OCR 結果の JSONL ファイル（`OCR_multiple_image.py` の出力） |
| `XML_TXT` | ✅ | XML 抽出テキストファイル（`hokuto_scrape_xml_dump.py` の出力） |
| `OUTPUT_XLSX` | — | 出力 Excel パス（省略時: `<basename>_merged.xlsx`） |

### 実行例

```bash
# 出力ファイル名を明示指定
python hokuto_marge_ocr_xml_reviews.py \
    "さいたま市立病院_OCR.jsonl" \
    "さいたま市立病院_OCR.txt" \
    "さいたま市立病院_final.xlsx"

# 出力ファイル名を自動生成（→ さいたま市立病院_merged.xlsx）
python hokuto_marge_ocr_xml_reviews.py \
    "さいたま市立病院_OCR.jsonl" \
    "さいたま市立病院_OCR.txt"
```
...
```bash
wsl python /mnt/c/Users/<username>/Desktop/hokuto_marge_ocr_xml_reviews.py \
    "/mnt/c/Users/<username>/Desktop/さいたま市立病院/さいたま市立病院_OCR.jsonl" \
    "/mnt/c/Users/<username>/Desktop/さいたま市立病院/さいたま市立病院_OCR.txt" \
    "/mnt/c/Users/<username>/Desktop/さいたま市立病院/さいたま市立病院_final.xlsx"
```
...
```
📖 OCRデータを読み込み中: さいたま市立病院_OCR.jsonl
   47ページ分のデータを読み込みました

📖 XML抽出データを読み込み中: さいたま市立病院_OCR.txt
   523行のデータを読み込みました
...
🔗 レビューをマッチング中...
   OCRレビュー: 32件
   XMLレビュー: 30件
   ✓ マッチ: 2024年度 5年 (類似度: 87.32%) [XML高精度テキスト使用]
   ✓ マッチ: 2024年度 6年 (類似度: 92.15%) [XML高精度テキスト使用]
   ...
...
| 大学 | 学年 | 性別 | 参加形態 | 年度 | 良い点 | 気になる点 | データソース | 類似度 |
|------|------|------|----------|------|--------|-----------|-------------|--------|
| 東京大学 | 5年 | 男性 | 見学した | 2024年度 | 指導医が... | 当直が... | OCR structure + XML text | 87.32% |

### Excel — Hospital Info シート
...
スクリプト冒頭の定数で挙動を切り替えられます：

```python
TRUNCATE_AFTER_REPORT_LINE = True   # 「口コミの問題を報告」以降を捨てる
REMOVE_MD_HEADINGS = True           # ## 良い点 / ## 気になる点 の行を消す
STRICT_UNIVERSITY_FILTER = True     # 「大学名っぽくないもの」は空欄にする
```

### マージ設定

```python
MERGE_ADJACENT_OVERLAP = True       # 分裂レビューの自動結合
MIN_OVERLAP_NORM_CHARS = 40         # 重複とみなす最小一致長
FUZZY_OVERLAP_RATIO = 0.93          # OCR 誤差の許容閾値
HIGH_SIM_RATIO = 0.92               # ほぼ同一文章の判定閾値
```
...
### 1. メタデータマッチング
年度 + 学年 + 参加形態が一致する OCR/XML レビューをペアリング

### 2. テキスト類似度計算
`difflib.SequenceMatcher` で OCR テキストと XML テキストの類似度を計算（閾値: 30%）

### 3. 見出し比率分割
マッチしたペアに対し、OCR の「良い点」と「気になる点」の文字数比率で XML テキストを分割

### 4. 分裂レビュー結合
ページまたぎで分裂した同一レビューを、末尾↔先頭のオーバーラップ検出（fuzzy matching 対応）で結合

### 5. 重複除去
ほぼ同一のレビュー（類似度 92% 以上）は長い方を残して除去
...
| パッケージ | 用途 |
|-----------|------|
| [openpyxl](https://openpyxl.readthedocs.io/) | Excel (.xlsx) 生成・スタイリング |
| [difflib](https://docs.python.org/3/library/difflib.html) | テキスト類似度計算（SequenceMatcher） |
| 標準ライブラリ (json, re, sys) | JSONL 読み込み、正規表現パーサ |

---

## 📌 注意事項

- 入力の JSONL は `{"file": "...", "text": "..."}` 形式を想定しています（`OCR_multiple_image.py` の出力と互換）
- XML テキストは `hokuto_scrape_xml_dump.py` で抽出した行ベースのテキストを想定
- マッチング精度は OCR の品質に依存します。HunyuanOCR の方が PaddleOCR より日本語精度が高い傾向があります
- Excel のセル文字数制限（32,000 文字）を超える場合は自動で切り詰められます
